...
def handle_sigint(sig_num: int, frame: FrameType | None) -> None:
    """Handle getting sigint to quit server."""
    # Since disconnect is async, we have to call it from event loop
    nicegui.ui.timer(0.1, disconnect, once=True)
...
def setup() -> None:
    ...
    async def grab_frame() -> fastapi.Response:
        ...
        frame = await nicegui.run.io_bound(camera.get_frame)
    ...
    nicegui.ui.button("Take Photo", on_click=camera.take_photo)
    ...
    nicegui.app.on_shutdown(cleanup)
