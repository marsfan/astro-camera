...
# Something like this:
# fourcc = cv2.VideoWriter_fourcc(*"MJPG")                noqa: ERA001
# camera.set(cv2.CAP_PROP_FOURCC, fourcc)                 noqa: ERA001
# assert fourcc == camera.get(cv2.CAP_PROP_FOURCC)        noqa: ERA001
# camera.set(cv2.CAP_PROP_CONVERT_RGB, 0)                 noqa: ERA001
# On linux, we can list supported fourcc with
# `v4l2-ctl --device=/dev/video0 --list-formats-ext`
#
# Also can list all info about device with
# `v4l2-ctl --device=/dev/video0 --all`

# TODO: Support exposure, gain, etc
# cv2.CAP_PROP_EXPOSURE controlss exposure. Small number is longer exposure
# (IDK why). Seems to range from 0 to 10000 in steps of 10?
...
class CameraThread(Thread):
    ...
    def run(self) -> None:
        ...
        # camera.set(cv2.CAP_PROP_AUTO_EXPOSURE, 1) But then I have to handle
        # controlling things myself. Instead, I can use this command to take
        # manual control and it seems to work out OK It still sort of manual,
        # but brightness looks better. Per RPI forums, this is to let the AE
